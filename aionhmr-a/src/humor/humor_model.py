...
def step(
    ...
    betas_in = meta["betas"].reshape((B, T, 1, -1)).expand((B, T, S_out, 10)).to(device)
    betas_in = betas_in.reshape((B * T * S_out, 10))
    ...
    def __init__(
        ...
        if self.use_smpl_joint_inputs:
            ...
            self.male_bm = BodyModel(
                bm_path=male_bm_path, num_betas=10, batch_size=self.smpl_batch_size
            )
            female_bm_path = os.path.join(SMPLH_PATH, "female/model.npz")
            self.female_bm = BodyModel(
                bm_path=female_bm_path, num_betas=10, batch_size=self.smpl_batch_size
            )
            neutral_bm_path = os.path.join(SMPLH_PATH, "neutral/model.npz")
            self.neutral_bm = BodyModel(
                bm_path=neutral_bm_path, num_betas=10, batch_size=self.smpl_batch_size
            )
            # self.male_bm = BodyModel(
