...
class Params(nn.Module):
    ...
    def set_require_grads(self, names):
        ...
        for name in names:
            ...
            #     if isinstance(betas, torch.Tensor) and betas.requires_grad:
            #         def mask_betas_gradient(grad):
            #             mask = torch.zeros_like(grad)
            #             mask[:, :10] = 1  # keep only first 10
            #             return grad * mask
            #         betas.register_hook(mask_betas_gradient)
