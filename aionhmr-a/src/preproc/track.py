...
class HMR2_4dhuman(PHALP):
    ...
    def setup_hmr(self):
        ...
        self.ViTPose = ViTPoseModel("cuda:0")
        self.HMAR = HMR2023TextureSampler(self.cfg).to("cuda:0")
    ...
    def run_additional_models(
        ...
        keypoint = np.array(vitposes_list)
        check = 1
        for person_id in range(keypoint.shape[0]):
            keypoints = keypoint[[person_id]].reshape((25,3))
            if keypoints[0,2] > 0.7 and keypoints[2,2] > 0.7 and keypoints[15,2] > 0.7 and keypoints[15,2] > 0.7 and keypoints[16,2] > 0.7 and  keypoints[17,2] > 0.7 and keypoints[18,2] > 0.7:
                check *= 1
            else:
                check *= 0 
