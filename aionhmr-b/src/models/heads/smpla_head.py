...
def build_smpla_head(cfg):
    smpla_head_type = cfg.MODEL.SMPL_HEAD.get('TYPE', 'hmr')
    if  smpla_head_type == 'transformer_decoder':
        return SMPLATransformerDecoderHead(cfg)
    else:
        raise ValueError('Unknown SMPL head type: {}'.format(smpla_head_type))

class SMPLATransformerDecoderHead(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.SMPL_HEAD.get('INIT_DECODER_XAVIER', False):
            ...
            # nn.init.xavier_uniform_(self.decalpha.weight, gain=0.01)
            nn.init.xavier_uniform_(self.deccam.weight, gain=0.01)

        mean_params = np.load(cfg.SMPL.MEAN_PARAMS)
        zer = torch.zeros(11)
        init_body_pose = torch.from_numpy(mean_params['pose'].astype(np.float32)).unsqueeze(0)
        init_betas = torch.from_numpy(mean_params['shape'].astype('float32'))
        zer[:10] = init_betas
        init_betas = zer.unsqueeze(0)
