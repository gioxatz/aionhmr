...
class SMPL(smplx.SMPLLayer):
    def __init__(self, *args, joint_regressor_extra: Optional[str] = None, update_hips: bool = False, **kwargs):
        """
        Extension of the official SMPL implementation to support more joints and SMPL-A model.
        Args:
            Same as SMPLLayer.
    ...
    def forward(self, *args, **kwargs) -> SMPLOutput:
        ...
        """
        smpla_output = super(SMPL, self).forward(*args, **kwargs)
        joints = smpla_output.joints[:, self.joint_map, :]
        if self.update_hips:
            joints[:,[9,12]] = joints[:,[9,12]] + \
                0.25*(joints[:,[9,12]]-joints[:,[12,9]]) + \
                0.5*(joints[:,[8]] - 0.5*(joints[:,[9,12]] + joints[:,[12,9]]))
        if hasattr(self, 'joint_regressor_extra'):
            extra_joints = vertices2joints(self.joint_regressor_extra, smpla_output.vertices)
            joints = torch.cat([joints, extra_joints], dim=1)
        smpla_output.joints = joints
        return smpla_output
