...
class ImageDataset(Dataset):
    ...
    def __init__(self,
        ...
        try:
            ...
            if self.betas.shape[1] == 10:
                if self.data['has_betas'].astype(int) == 0:
                    extra_beta = torch.tensor([0.0], dtype=torch.float32, requires_grad=False)
                    self.betas = torch.cat((self.betas, extra_beta), dim=0)
                else:
                    self.betas = np.concatenate((self.betas, np.zeros((1,), dtype=np.float32)), axis=0)
            self.has_betas = self.data['has_betas'].astype(np.float32)
        except KeyError:
            self.betas = np.zeros((len(self.imgname), 11), dtype=np.float32)
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        if betas.shape[0] == 10:
            if int(self.has_betas[idx].copy()) == 0:
                extra_beta = torch.tensor([0.0], dtype=torch.float32, requires_grad=False)
                betas = torch.cat((betas, extra_beta), dim=0)
            else:
                betas = np.concatenate((betas, np.zeros((1,), dtype=np.float32)), axis=0)
        ...
        for k, val in item.items():
            if isinstance(val, np.ndarray):
                if np.issubdtype(val.dtype, np.number):
                    item[k] = torch.from_numpy(val).contiguous()
                else:
                    item[k] = val
            elif isinstance(val, np.str_):
                item[k] = val
            elif isinstance(val, (np.generic, numbers.Number)):  
                item[k] = torch.tensor(val)
            else:
                item[k] = val
    ...
    def process_webdataset_tar_item(item, train, 
        ...
        betas = data['betas']
        # make it sure that betas are of shape (11,)
        if betas.shape[0] == 10: 
            if int(data['has_betas']) == 0:
                extra_beta = torch.tensor([0.0], dtype=torch.float32, requires_grad=False)
                betas = torch.cat((betas, extra_beta), dim=0)
            else:
                betas = np.concatenate((betas, np.zeros((1,), dtype=np.float32)), axis=0)
        ...
        for k, val in item.items():
            if isinstance(val, np.ndarray):
                item[k] = torch.from_numpy(val).contiguous()
            elif isinstance(val, (np.generic, numbers.Number)):  # numpy scalars and Python scalars
                item[k] = torch.tensor(val)
