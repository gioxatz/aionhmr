...
# Directory containing the SMPL-X KID model files (e.g., SMPLX_KID.npz)
SMPLX_MODEL_PATH = '/gpu-data/gchatz/smplify-x/models/smpl/SMPL_NEUTRAL.pkl'
# =================================
...
class Evaluator:
    ...
    def __init__(self,
                 ...
                 keypoint_list: List,
                 pelvis_ind: int,
                 metrics: List = ['mode_mpjpe', 'mode_re', 'min_mpjpe', 'min_re', 'avg_height_perc_diff', 'avg_height_diff', 'avg_height', 'depth', 'gt_depth', 'depth_diff'],
    ...
    def log(self):
        ...
        for metric in self.metrics:
            if metric in ['mode_mpjpe', 'mode_re', 'min_mpjpe', 'min_re']:
                unit = 'mm'
            elif metric in ['avg_height_diff', 'avg_height', 'depth', 'gt_depth', 'depth_diff']:
                unit = 'm'
            elif metric == 'avg_height_perc_diff':
                unit = '%'
            else:
                unit = ''
    ...
    def __call__(self, output: Dict, batch: Dict, opt_output: Optional[Dict] = None):
        ...
        gt_betas = batch['smpl_params']['betas'].detach().cpu().numpy()

        gt_pose = batch['smpl_params']['body_pose'].detach().cpu().numpy()

        gt_orient = batch['smpl_params']['global_orient'].detach().cpu().numpy()

        pred_betas = output['pred_smpl_params']['betas'].detach().cpu().numpy()

        body_pose = batch['smpl_params']['body_pose'].detach().cpu().numpy()

        global_orient = batch['smpl_params']['global_orient'].detach().cpu().numpy()

        if (hasattr(self, 'avg_height_perc_diff') or hasattr(self, 'avg_height_diff') or hasattr(self, 'avg_height')) or hasattr(self, 'depth'):
            try:
                if os.path.exists(SMPLX_MODEL_PATH)  :
                    ...
                    print(f"WARNING: SMPL-X model path not found at {SMPLX_MODEL_PATH}. 3D keypoints will be zeros.")
            except Exception as e:
                print(f"Error initializing SMPL-X Kid Model: {e}")
                smpl_model = None
                smpla_model = None  

        if hasattr(self, 'depth'): 

            # 2. Convert numpy arrays to PyTorch tensors and run model
            with torch.no_grad():
                global_orient_t = torch.from_numpy(global_orient).float()
                body_pose_t = torch.from_numpy(body_pose).float()
                global_orient_gt = torch.from_numpy(gt_orient).float()
                body_pose_gt = torch.from_numpy(gt_pose).float()
                pred_betas_t = torch.from_numpy(pred_betas).float()
                gt_betas_t = torch.from_numpy(gt_betas).float()
                
                smpl_output_pred = smpla_model(
            ...
            # 3. Extract the first 24 joints (standard SMPL body joints)
            verts_all_pred = smpl_output_pred.vertices.cpu().numpy()

            verts_all_gt = smpl_output_gt.vertices.cpu().numpy()

            z_verts_pred = verts_all_pred[:,:,-1]

            avg_z = np.max(z_verts_pred, axis=1)

            z_verts_gt = verts_all_gt[:,:,1]

            z_diff = z_verts_pred - z_verts_gt

            avg_z_gt = np.mean(z_verts_pred, axis=1)

        global_orient = np.zeros((batch_size, 3))
        body_pose = np.zeros((batch_size, 69))    

        if hasattr(self, 'avg_height_perc_diff'): 
            ...
            y_verts_pred = verts_all_pred[:,:,1]

            max_y = np.max(y_verts_pred, axis=1)

            min_y = np.min(y_verts_pred, axis=1)

            heights_pred = max_y - min_y


            y_verts_gt = verts_all_gt[:,:,1]

            max_y = np.max(y_verts_gt, axis=1)

            min_y = np.min(y_verts_gt, axis=1)

            heights_gt = max_y - min_y

            # 1. Get the top of the head (Max Y)
            # verts_all_pred shape is (Batch, 6890, 3)
            v_top_head_y = np.max(verts_all_pred[:, :, 1], axis=1) 

            # 2. Use the standardized SMPL Joints for the base points
            # Joint 12 is the Neck, Joint 0 is the Pelvis/Root
            joints = smpl_output_pred.joints.cpu().numpy()
            v_head_y = joints[:, 15, 1]
            v_neck_y = joints[:, 12, 1]
            v_pelvis_y = joints[:, 1, 1]

            # 3. Calculate lengths along the vertical axis
            # Head: Top of crown to Neck joint
            head_length = v_top_head_y - v_head_y

            # Torso: Neck joint to Pelvis joint
            torso_length = v_neck_y - v_pelvis_y

            # 4. Final Ratio
            head_to_torso = head_length / torso_length
        ...
        if hasattr(self, 'avg_height_perc_diff'):
            abs_percentage_error = (heights_gt - heights_pred) / heights_gt
            avg_height_perc_diff = np.mean(abs_percentage_error) * 100
            self.avg_height_perc_diff[self.counter:self.counter+batch_size] = avg_height_perc_diff
        if hasattr(self, 'avg_height_diff'):
            abs_error = (heights_gt - heights_pred)
            avg_height_diff = np.mean(abs_error)
            self.avg_height_diff[self.counter:self.counter+batch_size] = avg_height_diff
        if hasattr(self, 'avg_height'):
            abs_error1 = heights_pred
            avg_height = np.mean(abs_error1)
            self.avg_height[self.counter:self.counter+batch_size] = avg_height
        if hasattr(self, 'depth'):
            depth = np.mean(z_verts_pred)
            self.depth[self.counter:self.counter+batch_size] = depth
        if hasattr(self, 'gt_depth'):
            gt_depth = np.mean(z_verts_gt)
            self.gt_depth[self.counter:self.counter+batch_size] = gt_depth
        if hasattr(self, 'depth_diff'):
            depth_diff = np.mean(z_diff)
            self.depth_diff[self.counter:self.counter+batch_size] = depth_diff
        ...
        if hasattr(self, 'avg_height_perc_diff') and hasattr(self, 'avg_height_diff') and hasattr(self, 'avg_height'):
            return {
                'avg_height_perc_diff': avg_height_perc_diff,
